# Product Design QA — 通しプレイ・遷移・序盤テンポ（2026-08-23）

- browser path: fresh save、390×844 → 844×390、都市マップ → グリダニア全商戦 → リムサ・ロミンサ初戦
- late-game sample: published save 17/22、PC wide + 390×844、高難度ハブと絶カード
- automated coverage: normal/high-end progression checks、readiness、balance、500戦 simulation

## Observed flow and changes

1. 序盤マップ、対象選択、準備、結果、利益配分、次都市への移動に進行不能はない。縦持ちと低背横持ちの主要操作は画面内に収まる。
2. 毎戦の開始告知3.8秒は全編で反復すると重い。原作型の開始の間を残しつつ2.0秒へ短縮し、競合資本の積載へ早く移る。
3. 最初の都市ボスで味方人脈を学ぶ決着後、操作不能の所有率掃引が約18〜20秒続いた。結果とコイン演出は変えず、チュートリアル決着だけ最低18 gauge/sへ補正し、実ブラウザ再計測で支援選択から約7.7秒に収めた。
4. 市場価格の10%以下の少額投入でincoming束が二重化し、158ギルが序盤に不相応な横幅へ膨らんだ。10%以下は1束、25%以下は2束、それ以上は3束に固定した。
5. 都市解放直後のSYNERGYは既に自動装備されているのにアビリティ画面へ強制移動していた。SYNERGY解説は「装備済み・商戦へ戻る」で市場に留め、手動設定が必要なアビリティだけ台帳へ案内する。
6. 中断商戦は保存時の物件スナップショットでなく、再開時の現行ID定義へ再結合する。保存不能時は商戦開始を止めて警告し、ニューゲーム削除失敗時も再読み込みしない。
7. 開始画面と商戦モーダルのTab循環を補強し、エンディング遅延読込中もクリック可能な空白状態を作らない。
8. 既存500戦は任意カルテル6社を含む全22社の人脈を暗黙に使っていた。必須16社・外部同盟なしでは零式1編3層が0/10となるため、零式／幻の必須物件人脈へ「対象相場の50%を基準にして既存×0.5補正と共通減衰を適用」する最低線を追加した。任意カルテル6社は従来どおり物件相場から支援額を求めるため、その取得価値を残しつつ、絶・酷・業の既存支援量は変えない。
9. 新しい120戦の段階別監査を追加。通常編クリア直後の必須16社・外部同盟なしで零式1編1〜4層が40/40勝、各p90は20.4／40.0／52.5／74.2秒。後続8層も記録し、全取得済み500戦の既存勝率・酷・絶の所要時間は維持した。

## Remaining evidence limits

- 実操作は序盤2都市目までと公開済み後半セーブの代表画面。全22戦の状態遷移は自動進行検査、固定500戦、通常編クリア直後の追加120戦で補完する。
- スクリーンショットは静止状態のレイアウト証拠であり、実機GPU、音響、完全なWCAG適合を単独では証明しない。
- 外部のロマサガ3／FF14ファンへ実際に配布した第三者アンケートではなく、独立したコード・UX・バランス監査として記録する。
