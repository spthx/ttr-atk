...
> **2026-08-22更新:** コイン表示の旧24列・99ms・active/banked page数値契約は廃止した。移植時は[`romasaga3-trade-reference.md`](./romasaga3-trade-reference.md)と[`rs3-trade-regression-baseline.json`](./rs3-trade-regression-baseline.json)を正本にし、18アンカー `[4,5,5,4]`、最低8枚束、165ms×最大9wave、SFC表示ではbankingなしを再現する。
...
- 現行SFC表示は完成山をbankせず、台座を下降させない。投入額によるDOMやpoolの増減も行わない
...
- 長柱は同じ18アンカーを上へ伸ばし、field上端でclipする。リマスター式下降は別プリセットとして実装・監査するまで混ぜない。
