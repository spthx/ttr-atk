...
> **2026-08-23更新:** 移植時は[`romasaga3-trade-reference.md`](./romasaga3-trade-reference.md)と[`rs3-trade-regression-baseline.json`](./rs3-trade-regression-baseline.json)を正本にし、18アンカー `[4,5,5,4]`、最低8枚束、165ms×最大9waveに加え、過積載時の「完成柱＋物理台座の一体下降、固定した上段への継続補充、下段柱の一枚刻み表示」を再現する。

## 1. 目的と非目的

2026-08-30の実装は[描画更新記録](./renderer-upgrade-2026-08-30.md)を優先する。素材をテーマ定義へ分離し、柱のbitmap再利用と共通viewport下降を採用した。旧banked DTOをゲーム計算へ戻す方式ではない。商用移植に必要な権利・配布・保存分離は[商用版準備仕様](./commercial-edition-readiness.md)を参照する。
...
- 18本の固定アンカーを奥から `[4,5,5,4]` に配置し、1列最大512論理層。長柱は上端でclipする
- 少額は前中央から連続して埋め、左右は同じ幾何を鏡像にする
- 1以上の論理unitは最低8枚の仕切りが読める短い円柱束として描く
- 通常設定の1waveは165ms、1回の通常投入は最大9wave、落下束は4論理層
- 過積載時は完成山と物理台座を同じ移動体として一定量下降させ、固定した上段active基準線へ次の束を補充する。投入額によるDOMやpoolの増減は行わない
- 相場35%の全力投入を1つの視覚ページ基準として投入額を列高へ連続的に変換する

Unityではsettled 18アンカーとincoming 18アンカーの固定プールをあらかじめ生成し、各列の表示層またはスプライト状態だけを更新する。金額が増えてもpoolを増やさず、timelineのwaveを順に再利用する。互換DTOにbanked系フィールドが残っていても、現行SFC rendererでは中立値として無視する。

### 5.2 見た目の要件

- 少額は前中央から支持された塊として積み、後方に孤立した束や薄い1枚を残さない。
- incoming waveは最大18アンカーへ4論理層の短い円柱束を落とす165msのビートとし、投入額に応じた最大9waveを途切れず積む。モーション低減時だけ1waveへ圧縮する。
- 拡縮で金額を誤魔化さない。
- 金貨束は8本以上の水平仕切りを読み取れ、台座との背景ギャップを1 device px以内にする。
- 台座背面、settled、incoming、台座前壁の順で描き、前列の根元を前壁へ少し隠す。
- 長柱は同じ18アンカーを上へ伸ばし、上限超過分は完成柱と物理台座を一体で下段へ送る。下段柱の水平仕切りを維持し、暗い連続面や数値だけの要約へ置換しない。
...
7. 18アンカーsettled＋18アンカーincomingの固定プール、厚い台座の前後分割、最低8枚束、上端clipを追加する。
