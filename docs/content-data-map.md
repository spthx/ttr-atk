...
|業商戦の記憶・ものまね|`src/utils/karmaBattle.ts`|`業商戦：値札のない一株`の現在1件だけの記憶、4回の写し、無銘口座|
...
4. `src/utils/karmaBattle.ts`: 業商戦の名称、現在1件だけの記憶、ものまねの系統と無銘口座
...
- セーブと中断復帰が二重精算を起こさない。
- 業商戦と幻・商戦は酷商戦踏破後に並行して解放される。業商戦は開幕50%から55%・70%・85%・95%へ進めた節目ごとに、その時の一手を1件だけ記憶して6秒予告でものまねする。対処後はその記憶を消し、次の節目へ進むため、同時に覚える対象は常に1件だけ。指定二系統は完全取消、ほかの別系統は50%、同系統・無行動は100%着弾。競合予算の24%は開始時に無銘口座へ隔離され、途中の隠し増資には使わない。
