...
export const BeginnerGuide: React.FC = () => (
  <details className="group rounded-xl border border-cyan-500/25 bg-cyan-950/10 text-xs text-slate-200">
    <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-2.5 font-bold text-cyan-200 marker:content-none">
      <span className="flex items-center gap-2">
        <BookOpenCheck className="h-4 w-4 text-cyan-400" />
        はじめての方へ：最初の買収まで
      </span>
      <span className="text-[10px] font-normal text-slate-500 group-open:hidden">必要なときだけ開く</span>
      <span className="hidden text-[10px] font-normal text-slate-500 group-open:inline">閉じる</span>
    </summary>
    ...
    <div className="border-t border-cyan-500/20 px-3 pb-3 pt-2.5">
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
        ...
        <div className="rounded-lg border border-slate-800 bg-slate-950/75 p-2.5">
          ...
          <p className="leading-relaxed text-slate-400">取得物件から毎秒収益が入り、次の買収とネマワシへ使えます。</p>
        </div>
      </div>

      <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 rounded-lg bg-slate-950/60 px-2.5 py-2 text-[11px] text-slate-400">
        ...
        <span className="ml-auto text-[10px] text-cyan-400/80">「？」はマウスオーバー／タップで説明</span>
      </div>
    </div>
  </details>
);
